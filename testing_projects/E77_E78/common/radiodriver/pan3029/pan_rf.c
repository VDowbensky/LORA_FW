...
*/
volatile RfRxPkt_t g_RfRxPkt[PAN_COUNT] = {0};
...
*/
static volatile RfConfig_t g_RfCfgParams[PAN_COUNT] = {0};

/**
* @brief Save the current RF operation state
*/
static volatile RfOpState_t g_RfOperatetate[PAN_COUNT];
...
*/
PAN_Err_t PAN_SetPage(uint8_t pan,uint8_t Page)
{
  static uint8_t gCurrPage[PAN_COUNT] = {0xFF};
  if(gCurrPage[pan] == Page) return PAN_OK;
  gCurrPage[pan] = Page;
  PAN_ASSERT(PAN_WriteReg(pan,0x00, gCurrPage)); /* Select register page */
  return PAN_OK;
}
...
* - PAN_FAIL: Operation failed
*/
PAN_Err_t PAN_WritePageReg(uint8_t pan,uint8_t Page, uint8_t Addr, uint8_t Value)
{
  PAN_SetPage(uint8_t pan,Page);
  PAN_WriteReg(uint8_t pan,Addr, Value);
  return PAN_OK;
}
...
*/
void PAN_WritePageRegs(uint8_t pan, uint8_t Page, uint8_t Addr, uint8_t *Buffer, uint8_t Size)
{
  PAN_SetPage(pan,Page);                 /* Select register page */
  PAN_WriteRegs(pan, Addr, Buffer, Size); /* Write register value */
}
...
*/
uint8_t PAN_ReadPageReg(uint8_t pan, uint8_t Page, uint8_t Addr)
{
  PAN_SetPage(pan,Page);
  return PAN_ReadReg(pan,Addr);
}
...
*/
void PAN_ReadPageRegs(uint8_t pan, uint8_t Page, uint8_t Addr, uint8_t *Buffer, uint8_t Size)
{
  PAN_SetPage(pan,Page);                /* Select register page */
  PAN_ReadRegs(pan, Addr, Buffer, Size); /* Read register value */
}
...
* - PAN_FAIL: Operation failed
*/
PAN_Err_t PAN_SetPageRegBits(uint8_t pan, uint8_t Page, uint8_t Addr, uint8_t Mask)
{
  uint8_t Temp;

  PAN_SetPage(pan,Page);
  Temp = PAN_ReadReg(pan,Addr);
  PAN_WriteReg(pan,Addr, Temp | Mask);
  return PAN_OK;
}
...
* - PAN_FAIL: Operation failed
*/
PAN_Err_t PAN_ResetPageRegBits(uint8_t pan, uint8_t Page, uint8_t Addr, uint8_t Mask)
{
  uint8_t Temp;

  PAN_SetPage(pan,Page);                  /* Select register page */
  Temp = PAN_ReadReg(pan,Addr);           /* Read register value */
  PAN_WriteReg(pan,Addr, Temp & (~Mask)); /* Clear the register bits corresponding to the mask */
  return PAN_OK;
}
...
*/
PAN_Err_t PAN_WritePageRegBits(uint8_t pan, uint8_t Page, uint8_t Addr, uint8_t Value, uint8_t Mask)
{
  uint8_t Temp;
  ...
  PAN_SetPage(pan,Page);                            /* Select the register page */
  Temp = PAN_ReadReg(pan,Addr);                     /* Read the register value */
  PAN_WriteReg(pan, Addr, (Temp & (~Mask)) | Value); /* Clear the register bits corresponding to the mask and then set the new value */
  return PAN_OK;
}
...
* - PAN_FAIL: Operation failed
*/
PAN_Err_t PAN_ConfigGpio(uint8_t pan, uint8_t GpioPin, uint8_t GpioMode)
{
  if(GpioMode == PAN_GPIO_MODE_INPUT)
  {
    if(GpioPin < 8) PAN_ASSERT(PAN_SetPageRegBits(pan, 0, 0x63, (1 << GpioPin)));
    else PAN_ASSERT(PAN_SetPageRegBits(pan, 0, 0x64, (1 << (GpioPin - 8))));
  }
  else if(GpioMode == PAN_GPIO_MODE_OUTPUT)
  {
    if(GpioPin < 8) PAN_SetPageRegBits(pan, 0, 0x65, (1 << GpioPin));
    else PAN_SetPageRegBits(pan, 0, 0x66, (1 << (GpioPin - 8)));
  }
...
* - PAN_FAIL: Operation failed
*/
PAN_Err_t PAN_WriteGpioLevel(uint8_t pan, uint8_t GpioPin, uint8_t Level)
{
  if(GpioPin < 8) PAN_WritePageRegBits(pan, 0, 0x67, Level, (1 << GpioPin));
  else PAN_WritePageRegBits(pan, 0, 0x68, Level, (1 << (GpioPin - 8)));
  return PAN_OK;
}
...
* - 1: High level
*/
uint8_t PAN_ReadGpioLevel(uint8_t pan, uint8_t GpioPin)
{
  uint8_t Temp;

  if(GpioPin < 6) Temp = PAN_ReadPageReg(pan, 0, 0x74);
  else
  {
    Temp = PAN_ReadPageReg(pan, 0, 0x75);
    GpioPin -= 6;
  }
...
*/
void PAN_InitAntGpio(uint8_t pan)
{
  PAN_ConfigGpio(pan, MODULE_GPIO_RX, PAN_GPIO_MODE_OUTPUT);
  PAN_ConfigGpio(pan, MODULE_GPIO_TX, PAN_GPIO_MODE_OUTPUT);
  PAN_WriteGpioLevel(pan, MODULE_GPIO_RX, 0);
  PAN_WriteGpioLevel(pan, MODULE_GPIO_TX, 0);
}
...
* @note If you use the MCU's GPIO to control the antenna switch, you need to re-adapt this function.
*/
void PAN_TurnonTxAnt(uint8_t pan)
{
  PAN_WriteGpioLevel(pan, MODULE_GPIO_RX, 0);
  PAN_WriteGpioLevel(pan, MODULE_GPIO_TX, 1);
}
...
*/
void PAN_TurnonRxAnt(uint8_t pan)
{
  PAN_WriteGpioLevel(pan, MODULE_GPIO_TX, 0);
  PAN_WriteGpioLevel(pan, MODULE_GPIO_RX, 1);
}
...
* @note If you use the MCU's GPIO to control the antenna, you need to reconfigure this function.
*/
void PAN_ShutdownAnt(uint8_t pan)
{
  PAN_WriteGpioLevel(pan, MODULE_GPIO_RX, 0);
  PAN_WriteGpioLevel(pan, MODULE_GPIO_TX, 0);
}
...
*/
void PAN_InitTcxoGpio(uint8_t pan)
{
    PAN_ConfigGpio(pan, MODULE_GPIO_TCXO, PAN_GPIO_MODE_OUTPUT);
    PAN_WriteGpioLevel(pan, MODULE_GPIO_TCXO, 1);
}
...
*/
void PAN_TurnonTcxo(uint8_t pan)
{
  PAN_WriteGpioLevel(pan, MODULE_GPIO_TCXO, 1);
}
...
* @note If you use the MCU's GPIO to control the TCXO on/off, you need to reconfigure this function.
*/
void PAN_TurnoffTcxo(uint8_t pan)
{
  PAN_WriteGpioLevel(pan, MODULE_GPIO_TCXO, 0);
}

/**
* @brief Enable LDO PA
*/
void PAN_TurnonLdoPA(uint8_t pan)
{    
  PAN_SetPageRegBits(pan, 0, 0x4F, 0x08);
}

/*
* @brief Disable LDO PA
*/
void PAN_TurnoffLdoPA(uint8_t pan)
{
  PAN_ResetPageRegBits(pan, 0, 0x4F, 0x08);
}

/**
* @brief Turn off internal and external PAs
*/
void PAN_TurnoffPA(uint8_t pan)
{
  PAN_TurnoffLdoPA(pan); /* Turn off internal PAs */
  PAN_ShutdownAnt(pan);  /* Turn off external PAs */
  /* After transmission is complete, if configured in DCDC power mode, switch back to DCDC power mode. */
  if(g_RfCfgParams[pan].RegulatorMode == USE_DCDC) PAN_WritePageReg(pan, 3, 0x24, 0x08);
}

/**
* @brief Turn on internal and external PAs
*/
void PAN_TurnonPA(uint8_t pan)
{
  /* If the current power mode is DCDC, switch to LDO power mode before transmitting. */
  if(g_RfCfgParams[pan].RegulatorMode == USE_DCDC) PAN_WritePageReg(pan, 3, 0x24, 0x00);
  PAN_TurnonLdoPA(pan); /* Turn on the internal PA. */
  PAN_TurnonTxAnt(pan); /* Turn on the external PA. */
}
...
* - CHIPMODE_MODE1
*/
void PAN_SetChipMode(uint8_t pan, RfChipMode_t ChipMode)
{
  if(ChipMode[pan] == CHIPMODE_MODE0)
  {
    /* Mode0 config */
    PAN_WritePageRegBits(pan, 1, 0x25, 0, 0xF0);
    PAN_WritePageRegBits(pan, 1, 0x25, 0, 0x08);
    PAN_WritePageRegBits(pan, 3, 0x12, 1, 0x04);
    PAN_WritePageRegBits(pan, 3, 0x12, 1, 0x10);
    PAN_WritePageRegBits(pan, 0, 0x58, 1, 0x04); /* Enable crc interrupt */
  }
  else
  {
    /* Mode1 config */
    PAN_WritePageRegBits(pan, 1, 0x25, 4, 0xF0);
    PAN_WritePageRegBits(pan, 1, 0x25, 1, 0x08);
    PAN_WritePageRegBits(pan, 3, 0x12, 0, 0x04);
    PAN_WritePageRegBits(pan, 3, 0x12, 0, 0x10);
    PAN_WritePageRegBits(pan, 0, 0x58, 0, 0x04); /* Disable crc interrupt */
  }
  g_RfCfgParams[pan].ChipMode = ChipMode;
}
...
* @param -
*/
RfChipMode_t PAN_GetChipMode(uint8_t pan)
{
  return g_RfCfgParams[pan].ChipMode;
}
...
*/
uint8_t PAN_ReadInfoByte(uint8_t pan, uint8_t Addr, uint16_t Pattern, uint8_t InfoAddr)
{
  uint8_t Value;
  uint8_t Buffer[3];
  ...
  Buffer[2] = InfoAddr << 1;
  PAN_WritePageRegs(pan, 2, Addr, Buffer, sizeof(Buffer));
  do
  {
    if (PAN_ReadPageReg(pan, 0, 0x6C) & 0x80) break;
  } while (Timeout--);
  Value = PAN_ReadPageReg(pan, 2, Addr);
  return Value;
}

/**
* @brief Calibrate RF related parameters
* @return PAN_Err_t Return operation result
* - PAN_OK: Operation successful
* - PAN_FAIL: Operation failed
*/
PAN_Err_t PAN_Calibrate(uint8_t pan)
{
  int i;
  uint8_t Temp[3] = {0};
  ...
  PAN_ResetPageRegBits(pan, 2, 0x3E, 0x08); // Unlock info
  for (i = 0; i < sizeof(Temp); i++) Temp[i] = PAN_ReadInfoByte(pan, 0x3B, 0x5AA5, 0x1E + i);
  if (PAN_ReadInfoByte(0x3B, 0x5AA5, 0x1C) == 0x5A)
  {
    PAN_WritePageReg(pan, 2, 0x3D, 0xFD);
    if (Temp[2] != 0) PAN_WritePageReg(pan, 0, 0x45, Temp[2]); /* Write PABIAS */
    PAN_WritePageReg(pan, 3, 0x1C, (0xC0 | (Temp[0] & 0x1F))); /* Write DCDCIMAX */
    PAN_WritePageReg(pan, 3, 0x1D, Temp[1]);                   /* Write DCDCREF */
  }
  PAN_ASSERT(PAN_SetPageRegBits(pan, 2, 0x3E, 0x08)); // Lock info
  return PAN_OK;
}
...
* - PAN_FAIL: Operation failed
*/
PAN_Err_t PAN_ConfigDefaultParams(uint8_t pan)
{
  int i;
  for(i = 0; i < sizeof(g_RfDefaultConfig)/sizeof(PAN_RegCfg_t); i++)
  {
    PAN_WritePageReg(pan, g_RfDefaultConfig[i].Page, g_RfDefaultConfig[i].Addr, g_RfDefaultConfig[i].Value);
  }
  return PAN_OK;
}
...
*/
PAN_Err_t PAN_Init(uint8_t pan)
{
  ...
  PAN_WriteReg(pan, 0x04, 0x06); /* Start POR reset chip */
  PAN_DelayUs(100);         /* Ensure that the actual delay is above 100us */
#if INTERFACE_MODE == USE_SPI_4LINE
  PAN_WriteReg(pan, 0x00, 0x03); /* Select register page 3 */
  PAN_WriteReg(pan, 0x1A, 0x03); /* Enable 4-line SPI */
#elif INTERFACE_MODE == USE_SPI_3LINE
  PAN_WriteReg(pan, 0x00, 0x03); /* Select register page 3 */
  PAN_WriteReg(pan, 0x1A, 0x83); /* Enable 3-line SPI */
#endif
  PAN_SetPage(pan, 0);                                    /* Select register page 0 */
  PAN_ASSERT(pan, PAN_WriteReg(0x02, PAN_STATE_DEEPSLEEP)); /* Enter deepsleep state */
  PAN_DelayUs(10);                                   /* Ensure actual delay is above 10us */
  PAN_ASSERT(pan, PAN_WriteReg(0x02, PAN_STATE_SLEEP));     /* Enter sleep state */
  PAN_DelayUs(10);                                   /* Ensure that the actual delay is above 10us */
  PAN_ASSERT(pan, PAN_WritePageReg(3, 0x06, 0x20));        /* Enable ISO */
  PAN_DelayUs(10);                                   /* Ensure that the actual delay is above 10us */
  PAN_ASSERT(pan, PAN_WriteReg(0x02, PAN_STATE_STB1));      /* Enter stb1 state */
  PAN_DelayUs(10);                                   /* Ensure that the actual delay is above 10us */
#if USE_ACTIVE_CRYSTAL == 1                           /* If using an active crystal oscillator, you need to configure the TCXO GPIO pin */
  PAN_ASSERT(pan, PAN_WritePageReg(3, 0x26, 0xA0));        /* Enable core power and turn on the active crystal oscillator channel */
  PAN_DelayUs(100);                                  /* Ensure that the actual delay is above 100us */
  PAN_ASSERT(pan, PAN_WriteReg(0x04, 0x36));               /* Enable LFT and release POR reset */
  PAN_DelayMs(1);                                    /* Ensure that the actual delay is above 1ms */
  PAN_InitTcxoGpio(pan);                                /* Initialize TCXO GPIO pin */
#else
  PAN_ASSERT(pan, PAN_WritePageReg(3, 0x26, 0x20));        /* Enable core power */
  PAN_DelayUs(100);                                  /* Ensure that the actual delay is above 100us */
  PAN_ASSERT(pan, PAN_WriteReg(0x04, 0x36));               /* Enable LFT and release POR reset */
  PAN_DelayMs(1);                                    /* Ensure that the actual delay is above 1ms */
#endif
  PAN_ASSERT(pan, PAN_WriteReg(0x02, PAN_STATE_STB2));      /* Enter stb2 state */
  PAN_DelayMs(1);                                    /* Ensure that the actual delay is above 1ms */
  PAN_ASSERT(pan, PAN_WriteReg(0x02, PAN_STATE_STB3));      /* Enter stb3 state */
  PAN_DelayUs(100);                                  /* Ensure that the actual delay is above 100us */
  PAN_ASSERT(pan, PAN_ConfigDefaultParams());              /* Configure the default parameters of the RF register */
  PAN_ASSERT(pan, PAN_Calibrate());                        /* Calibrate RF related parameters */
  PAN_ASSERT(pan, PAN_ConfigAgc());                        /* Configure the AGC function */
  PAN_InitAntGpio(pan);                                 /* Initialize the antenna GPIO pinS */
  g_RfOperatetate[pan] = PAN_STATE_STB3;                  /* Set the current working state to STB3 */
  return PAN_OK;
}
...
*/
void PAN_ConfigUserParams(uint8_t pan)
{
  PAN_SetTxPower(pan,22);                    /* Set the power level */
  PAN_SetFreq(pan,PAN_FREQ_DEFAULT);          /* Set the frequency */
  PAN_SetBW(pan,PAN_BW_DEFAULT);              /* Set the bandwidth */
  PAN_SetSF(pan,PAN_SF_DEFAULT);              /* Set the spreading factor */
  PAN_SetCR(pan,PAN_CR_DEFAULT);              /* Set the channel coding rate */
  PAN_SetCRC(pan,PAN_CRC_DEFAULT);            /* Set the CRC check */
  PAN_SetLDR(pan,PAN_LDR_DEFAULT);            /* Set the low-rate mode */
  PAN_SetPreamLen(pan,PAN_PREAMBLE_DEFAULT);  /* Set the preamble length */
  PAN_SetInvertIQ(pan,PAN_IQ_INVERT_DEFAULT); /* Set IQ to non-inverted */
  PAN_SetRegulatorMode(pan,USE_LDO);         /* Set the chip to LDO power mode */
  PAN_SetChipMode(pan,CHIPMODE_MODE0);       /* Set the chip mode to MODE0 */
}

/**
* @brief Software reset of the RF chip control logic
*/
void PAN_ResetLogic(uint8_t pan)
{
  PAN_WriteReg(pan,0x00, 0x80);
  PAN_WriteReg(pan,0x00, 0x00);
  (void)PAN_ReadReg(pan,0x00); /* A dummy read of register 0x00 is required for the reset to take effect */ 
}
...
*/
RfOpState_t PAN_GetOperateState(uint8_t pan)
{
  return g_RfOperatetate[pan];
}
...
* - PAN_STATE_RX: Chip is in receive mode
*/
void PAN_SetOperateState(uint8_t pan, RfOpState_t RfState)
{
  g_RfOperatetate[pan] = RfState;
}
...
* - PAN_STATE_RX
*/
void PAN_SetRfState(uint8_t pan, uint8_t RfState)
{
  PAN_WriteReg(pan, 0x02, RfState);
  g_RfOperatetate[pan] = (RfOpState_t)RfState;
}
...
*/
void PAN_EnterDeepsleepState(uint8_t pan)
{
  PAN_ShutdownAnt(pan);                      /* Turn off the antenna */
  PAN_WriteReg(pan, 0x02, PAN_STATE_STB3);      /* Enter STB3 state */
  PAN_DelayUs(150);                       /* Ensure that the actual delay is above 150us */
  PAN_WriteReg(pan, 0x02, PAN_STATE_STB2);      /* Enter STB2 state */
  PAN_DelayUs(10);                        /* Ensure that the actual delay is above 10us */
  PAN_WriteReg(pan, 0x02, PAN_STATE_STB1);      /* Enter STB1 state */
  PAN_DelayUs(10);                        /* Ensure that the actual delay is above 10us */
...
#endif
  PAN_WriteReg(pan, 0x04, 0x06);               /* Turn off LFT */
  PAN_DelayUs(10);                        /* Ensure that the actual delay is above 10us */
  PAN_WriteReg(pan, 0x02, PAN_STATE_SLEEP);     /* Enter the SLEEP state */
  PAN_DelayUs(10);                        /* Ensure that the actual delay is above 10us */
  PAN_WritePageReg(pan, 3, 0x06, 0x00);        /* Turn off ISO */
  PAN_DelayUs(10);                        /* Ensure that the actual delay is above 10us */
  PAN_WritePageReg(pan, 3, 0x26, 0x00);        /* Turn off the internal power supply */
  PAN_DelayUs(10);                        /* Ensure that the actual delay is above 10us */
  PAN_WriteReg(pan, 0x02, PAN_STATE_DEEPSLEEP); /* Enter the DEEPSLEEP state */
  g_RfOperatetate[pan] = PAN_STATE_DEEPSLEEP;
}
...
*/
void PAN_EnterSleepState(uint8_t pan)
{
  PAN_ShutdownAnt(pan);                   /* Turn off the antenna */
  PAN_WriteReg(pan, 0x02, PAN_STATE_STB3);   /* Enter the STB3 state */
  PAN_DelayUs(150);                    /* Ensure that the actual delay is above 150us */
  PAN_WriteReg(pan, 0x02, PAN_STATE_STB2);   /* Enter STB2 state */
  PAN_DelayUs(10);                     /* Ensure that the actual delay is above 10us */
  PAN_WriteReg(pan, 0x02, PAN_STATE_STB1);   /* Enter STB1 state */
  ...
  PAN_TurnoffTcxo(pan);                   /* Turn off the TCXO power supply */
#endif
  PAN_WriteReg(pan, 0x04, 0x16);            /* Turn off LFT */
  PAN_DelayUs(10);                     /* Ensure the actual delay is above 10us */
  PAN_WriteReg(pan, 0x02, PAN_STATE_SLEEP);  /* Enter SLEEP state */
  PAN_DelayUs(10);                     /* Ensure the actual delay is above 10us */
  PAN_ResetPageRegBits(pan, 3, 0x06, 0x20); /* Turn off ISO */
  PAN_DelayUs(10);                     /* Ensure the actual delay is above 10us */
  PAN_WritePageReg(pan, 3, 0x26, 0x00);     /* Turn off internal power */
  g_RfOperatetate[pan] = PAN_STATE_SLEEP;
}
...
*/
void PAN_ExitSleepState(uint8_t pan)
{
  PAN_SetPageRegBits(pan, 3, 0x06, 0x20); /* Enable ISO */
  PAN_DelayUs(10);                   /* Ensure the actual delay is above 10us */
  PAN_WriteReg(pan, 0x02, PAN_STATE_STB1); /* Enter STB1 state */
  PAN_DelayUs(10);                   /* Ensure the actual delay is above 10us */
#if USE_ACTIVE_CRYSTAL == 1
  PAN_WritePageReg(pan, 3, 0x26, 0xA0);   /* Enable core power and turn on active crystal oscillator channel */
  PAN_DelayUs(100);                  /* Ensure the actual delay is above 100us */
  PAN_WriteReg(pan, 0x04, 0x36);          /* Enable LFT */
  PAN_DelayUs(100);                  /* Ensure the actual delay is above 100us */
  PAN_TurnonTcxo(pan);                  /* Turn on TCXO */
#else
  PAN_WritePageReg(pan, 3, 0x26, 0x20);   /* Enable core power */
  PAN_DelayUs(100);                  /* Ensure the actual delay is above 100us */
  PAN_WriteReg(pan, 0x04, 0x36);          /* Enable LFT */
  PAN_DelayUs(100);                  /* Ensure the actual delay is above 100us */
#endif
  ...
  PAN_WriteReg(pan, 0x02, PAN_STATE_STB3); /* Enter STB3 state */
  PAN_DelayUs(100);                  /* Ensure the actual delay is above 100us */
  g_RfOperatetate[pan] = PAN_STATE_STB3;
}
...
*/
void PAN_EnterStandbyState(uint8_t pan)
{
  PAN_SetRfState(pan, PAN_STATE_STB3);
  PAN_SetOperateState(pan, PAN_STATE_STB3);
}
...
* @note This function sets the chip's operating state to MODE_STDBY.
*/
void PAN_CheckDeviceReady(uint8_t pan)
{
  if (PAN_GetOperateState(pan) == PAN_STATE_SLEEP) PAN_ExitSleepState(pan);
}
...
*/
void PAN_SetRegulatorMode(uint8_t pan, RfRegulatorMode_t RegulatorMode)
{
  PAN_WritePageReg(pan, 3, 0x24, (RegulatorMode == USE_DCDC) ? 0x08 : 0x00);
  g_RfCfgParams[pan].RegulatorMode = RegulatorMode;
}
...
*/
PAN_Err_t PAN_SetFreq(uint8_t pan, uint32_t Frequency)
{
  int i;
  uint32_t Fa, Fb;
  uint32_t Temp = 0;
  ...
  for (i = 0; i < FreqTableNum; i++)
  {
    if (Frequency > g_RfFreqTable[i].StartFreq && Frequency <= g_RfFreqTable[i].StopFreq)
    {
      ...
      PAN_WritePageRegs(pan, 0, 0x40, (uint8_t *)&g_RfFreqTable[i].VcoParam, 2);
      PAN_WriteReg(pan, 0x3D, g_RfFreqTable[i].LoParam);
      break;
    }
  }
  ...
  PAN_WritePageRegs(pan, 3, 0x09, FreqReg, 4);
  Fab[0] = (uint8_t)(Fa);
  Fab[1] = (uint8_t)(Fb);
  Fab[2] = (uint8_t)((Fb >> 8) & 0x0F);
  PAN_WritePageRegs(pan, 3, 0x15, Fab, 3);
  g_RfCfgParams[pan].Frequency = Frequency;
  return PAN_OK;
}
...
*/
void PAN_SetInvertIQ(uint8_t pan, bool NewState)
{
  if (NewState)
  {
    /*
    ...
    * BIT5 = 1: invert tx IQ
    */
    PAN_WritePageRegBits(pan, 1, 0x0E, 0x01, 0x40 | 0x20);
    g_RfCfgParams[pan].InvertIQ = PAN_IQ_INVERTED;
  }
  else
  {
    /*
    ...
    */
    PAN_WritePageRegBits(pan, 1, 0x0E, 0x02, 0x40 | 0x20);
    g_RfCfgParams[pan].InvertIQ = PAN_IQ_NORMAL;
  }
}
...
* Range is 4 - 65535
*/
void PAN_SetPreamLen(uint8_t pan, uint16_t PreamLen)
{
  uint8_t Temp[2] = {(uint8_t)(PreamLen), (uint8_t)((PreamLen >> 8))};
  PAN_WritePageRegs(pan, 3, 0x13, Temp, 2);
  g_RfCfgParams[pan].PreambleLen = PreamLen;
}
...
*/
void PAN_SetSyncWord(uint8_t pan, uint8_t SyncWord)
{
  PAN_WritePageReg(pan, 3, 0x0F, SyncWord);
  g_RfCfgParams[pan].SyncWord = SyncWord;
}
 ...
 *|------|------------------|------------------|------------------|------------------|
 */
void PAN_SetTxPower(uint8_t pan, uint8_t TxPower)
{
  int Index;
  uint8_t Temp1, Temp2;
  ...
  PAN_WritePageReg(pan, 3, 0x22, g_RfPowerRampCfg[Index].Ldo & 0x01);
  PAN_WritePageReg(pan, 0, 0x1E, g_RfPowerRampCfg[Index].Ramp);
  PAN_WritePageReg(pan, 0, 0x4B, g_RfPowerRampCfg[Index].Ldo >> 4);
  if (g_RfPowerRampCfg[Index].PAbias != 0x70) PAN_SetPageRegBits(pan, 0, 0x46, 0x04); // page0, reg0x46, bit2=1
  else PAN_ResetPageRegBits(pan, 0, 0x46, 0x04); // page0, reg0x46, bit2=0
  if(!PaBiasReadFlag)
  {
    PaBiasReadFlag = true;
    PAN_ResetPageRegBits(pan, 2, 0x3E, 0x08); /* RF unlock info */
    PaBiasVal = PAN_ReadInfoByte(pan, 0x3B, 0x5AA5, 0x20);
    PAN_SetPageRegBits(pan, 2, 0x3E, 0x08);  /* RF lock info */
  ...
  PAN_WritePageReg(pan, 0, 0x45, Temp2);
  g_RfCfgParams[pan].TxPower = TxPower; /* save current TxPower value */
}
...
*/
void PAN_SetBW(uint8_t pan, uint8_t BandWidth)
{
  /* Page 3, Reg 0x0D, Bit[7:4] = BandWidth */
  PAN_WritePageRegBits(pan, 3, 0x0D, BandWidth, 0xF0);
  if (BandWidth != PAN_BW_500K) PAN_SetPageRegBits(pan, 2, 0x3F, 0x02);
  else PAN_ResetPageRegBits(pan, 2, 0x3F, 0x02);
  g_RfCfgParams[pan].Bandwidth = (RfBandwidths_t)BandWidth; // save current BW value
}
...
*/
void PAN_SetSF(uint8_t pan, uint8_t SpreadFactor)
{
  /* Page 3, Reg 0x0E, Bit[7:4] = SpreadFactor */
  PAN_WritePageRegBits(pan, 3, 0x0E, SpreadFactor, 0xF0);
  g_RfCfgParams[pan].SpreadingFactor = (RfSpreadFactor_t)SpreadFactor; // save current SF value
}
...
*/
void PAN_SetCR(uint8_t pan, uint8_t CodingRate)
{
  /* Page 3, Reg 0x0D, Bit[3:1] = CodingRate */
  PAN_WritePageRegBits(pan, 3, 0x0D, CodingRate, 0x0E);
  g_RfCfgParams[pan].CodingRate = (RfCodingRates_t)CodingRate; // save current CR value
}
...
*/
void PAN_SetCRC(uint8_t pan, uint8_t CrcMode)
{
  /* Page 3, Reg 0x0D, Bit[0] = CRC */
  PAN_WritePageRegBits(pan, 3, 0x0E, CrcMode, 0x08);
  g_RfCfgParams[pan].CrcMode = (RfCrcModes_t)CrcMode; // save current CRC value
}
...
*/
void PAN_SetLDR(uint8_t pan, uint8_t LdrMode)
{
  /* Page 3, Reg 0x12, Bit[3] = LDR */
  PAN_WritePageRegBits(pan, 3, 0x12, LdrMode, 0x08);
  g_RfCfgParams[pan].LowDatarateOptimize = LdrMode; // save current LDR value
}
...
*/
void PAN_SetModemMode(uint8_t pan, uint8_t ModemMode)
{
  if (ModemMode == MODEM_MODE_NORMAL) PAN_WritePageReg(pan, 1, 0x0B, 0x08);
  else if (ModemMode == MODEM_MODE_MULTI_SECTOR)
  {
    PAN_WritePageReg(pan, 1, 0x0B, 0x18);
    if( g_RfCfgParams[pan].SpreadingFactor <= PAN_SF6 )
    {
      PAN_WritePageReg(pan, 1, 0x2F, 0x74);
      PAN_WritePageReg(pan, 1, 0x30, 0x01);
    }
    else
    {
      PAN_WritePageReg(pan, 1, 0x2F, 0x54);
      PAN_WritePageReg(pan, 1, 0x30, 0x40);
    }
  }
}
...
*/
void PAN_SetTx(uint8_t pan, uint8_t *Buffer, uint8_t Size)
{
  PAN_TxSinglePkt(pan, Buffer, Size);
  g_RfOperatetate[pan] = PAN_STATE_TX;
}
...
*/
void PAN_SetRx(uint8_t pan, uint32_t TimeoutMs)
{
  if (TimeoutMs == 0) PAN_EnterContinousRxState(pan);
  else PAN_EnterSingleRxWithTimeout(pan, TimeoutMs);
  g_RfOperatetate[pan] = PAN_STATE_RX;
}
...
- PAN_CAD_04_SYMBOL 
*/
void PAN_StartCad(uint8_t pan, uint8_t Threshold, uint8_t Chirps)
{
  /* Configure GPIO11 as output for CAD indication */
  PAN_ConfigGpio(pan, MODULE_GPIO_CAD_IRQ, PAN_GPIO_MODE_OUTPUT);
  /* [Page0][Reg0x5E][Bit6] = 0, enable GPIO11 CAD indication */
  PAN_ResetPageRegBits(pan, 0, 0x5E, 0x40);
  PAN_WritePageReg(pan, 1, 0x0F, Threshold);            /* [Page1][Reg0x0F] = Threshold */
  PAN_WritePageRegBits(pan, 1, 0x25, Chirps - 1, 0x03); /* [Page1][Reg0x25][Bit[1:0]] = Chirps - 1 */
  PAN_WritePageReg(pan, 1, 0x35, 0xFE);                 /* [Page1][Reg0x35] payload cad config */
  PAN_EnterContinousRxState(pan);                     /* Enter continous RX state */
}
...
* - PAN_CAD_THRESHOLD_20 
*/
void PAN_SetCadThreshold(uint8_t pan, uint8_t Threshold)
{
  PAN_WritePageReg(pan, 1, 0x0F, Threshold);
}
...
* - PAN_CAD_04_SYMBOL
*/
void PAN_SetCadChirps(uint8_t pan, uint8_t Chirps)
{
  PAN_WritePageRegBits(pan, 1, 0x25, Chirps - 1, 0x03); /* [Page1][Reg0x25][Bit[1:0]] = Chirps */
}

/**
* @brief Stop CAD detection
*/
void PAN_StopCad(uint8_t pan)
{
    PAN_SetPageRegBits(pan, 0, 0x5E, 0x40); /* [Page0][Reg0x5E][Bit6] = 1, disable GPIO11 CAD indication */
    PAN_WritePageReg(pan, 1, 0x0F, 0x0A);   /* Reset CAD threshold */
    PAN_WritePageReg(pan, 1, 0x35, 0xF4);   /* Reset payload cad config */
    PAN_SetRfState(pan, PAN_STATE_STB3);     /* Enter standby state */
    PAN_ResetLogic(pan);                  /* Soft reset the RF chip, clear cad state */
    g_RfOperatetate[pan] = PAN_STATE_STB3;
}
...
*/
void PAN_SetTxMode(uint8_t pan, uint8_t TxMode)
{
  PAN_WritePageRegBits(pan, 3, 0x06, TxMode, 0x04); /* [Page3][Reg0x06][Bit[2]] = TxMode */
}
...
*/
void PAN_TxSinglePkt(uint8_t pan, uint8_t *Buffer, uint8_t Size)
{
  PAN_WriteReg(pan, 0x02, PAN_STATE_STB3); /* Enter standby state */
  PAN_SetTxMode(pan, PAN_TX_MODE_SINGLE);  /* Set single transmit mode */
  PAN_WritePageReg(pan, 1, 0x0C, Size);   /* Set transmit data length */
  PAN_TurnonPA(pan);                    /* Enable the PA before transmitting data */
  PAN_SetRfState(pan, PAN_STATE_TX);       /* Set the chip to transmit mode */
  PAN_WriteRegs(pan, 0x01, Buffer, Size); /* Write data to the FIFO. After writing the data, the chip starts transmitting data. 0x01 is the FIFO register address */
}
...
*/
void PAN_SetRxMode(uint8_t pan, uint8_t RxMode)
{
  PAN_WritePageRegBits(pan, 3, 0x06, RxMode, 0x03); /* [Page3][Reg0x06][Bit[1:0]] = RxMode */
}
...
*/
void PAN_EnterContinousRxState(uint8_t pan)
{
  PAN_SetRfState(pan, PAN_STATE_STB3);       /* Enter standby mode. */
  PAN_TurnonRxAnt(pan);                   /* Turn on the receive antenna. */
  PAN_TurnoffLdoPA(pan);                  /* Turn off the internal PA */
  PAN_SetRxMode(pan, PAN_RX_MODE_CONTINOUS); /* Set the receive mode to continuous receive */
  PAN_SetRfState(pan, PAN_STATE_RX);         /* Enter receive state */
}
...
*/
void PAN_SetRxTimeout(uint8_t pan, uint16_t TimeoutMs)
{
  uint8_t Temp[2] = {(uint8_t)TimeoutMs, (uint8_t)(TimeoutMs >> 8)};
  PAN_WritePageRegs(pan, 3, 0x07, Temp, 2);
}
...
*/
void PAN_EnterSingleRxWithTimeout(uint8_t pan, uint16_t TimeoutMs)
{
  PAN_SetRfState(pan, PAN_STATE_STB3);            /* Enter standby state */
  PAN_TurnonRxAnt(pan);                        /* Turn on the receive antenna */
  PAN_TurnoffLdoPA(pan);                       /* Turn off the internal PA */
  PAN_SetRxTimeout(pan, TimeoutMs);              /* Set the receive timeout */
  PAN_SetRxMode(pan, PAN_RX_MODE_SINGLE_TIMEOUT); /* Set the receive mode to single receive mode */
  PAN_SetRfState(pan, PAN_STATE_RX);              /* Enter receive state */
}
...
*/
uint8_t PAN_GetRxPayloadLen(uint8_t pan)
{
  return PAN_ReadPageReg(pan, 1, 0x7D);
}
...
*/
uint8_t PAN_GetRecvPayload(uint8_t pan, uint8_t *Buffer)
{
  uint8_t Size;
  
  Size = PAN_GetRxPayloadLen(pan);     /* Get the received data length */
  PAN_ReadRegs(pan, 0x01, Buffer, Size); /* Read the received data from the FIFO, where 0x01 is the FIFO register address */
  return Size;
}
...
*/
int8_t PAN_GetPktRssi(uint8_t pan)
{
  return PAN_ReadPageReg(pan, 1, 0x7F);
}
...
*/
int8_t PAN_GetRealTimeRssi(uint8_t pan)
{
  /* Clear Bit[2] of register 0x06 to 0 and then set it to 1 to update the value of [Page1][Reg0x7E] */
  PAN_ResetPageRegBits(pan, 2, 0x06, 0x04);
  PAN_SetPageRegBits(pan, 2, 0x06, 0x04);
  return (int8_t)PAN_ReadPageReg(pan, 1, 0x7E);
}
...
*/
int32_t PAN_GetPktSnr(uint8_t pan)
{
  ...
  PAN_ReadPageRegs(pan, 2, 0x71, &Temp[0], 3); // Noise strength
  PAN_ReadPageRegs(pan, 1, 0x74, &Temp[3], 3); // Singal strength
  ...
  if(g_RfCfgParams[pan].SpreadingFactor <= 9) SnrVal = (SingalStrength << (9 - g_RfCfgParams[pan].SpreadingFactor)) / NoiseStrength;
  else SnrVal = (SingalStrength >> (g_RfCfgParams[pan].SpreadingFactor - 9)) / NoiseStrength;
  for (i = 0; i < 31; i++)
  {
...
*/
uint8_t PAN_GetIRQFlag(uint8_t pan)
{
  return (PAN_ReadPageReg(pan, 0, 0x6C) & 0x7F);
}
...
*/
void PAN_ClrIRQFlag(uint8_t pan, uint8_t IRQFlag)
{
  PAN_WritePageReg(pan, 0, 0x6C, IRQFlag);
}

/**
* @brief Get the current frequency setting
*/
uint32_t PAN_GetFreq(uint8_t pan)
{
  return g_RfCfgParams[pan].Frequency;
}

/**
* @brief Get the current IQ inversion value
*/
RfIQModes_t PAN_GetInvertIQ(uint8_t pan)
{
  return g_RfCfgParams[pan].InvertIQ;
}

/**
* @brief Get the current preamble length setting
*/
uint16_t PAN_GetPreamLen(uint8_t pan)
{
  return g_RfCfgParams[pan].PreambleLen;
}

/**
* @brief Get the current transmit power setting
*/
uint8_t PAN_GetTxPower(uint8_t pan)
{
  return g_RfCfgParams[pan].TxPower;
}

/**
* @brief Get the current modulation bandwidth setting
*/
uint8_t PAN_GetBandWidth(uint8_t pan)
{
  return g_RfCfgParams[pan].Bandwidth;
}

/**
* @brief Get the current spreading factor setting
*/
uint8_t PAN_GetSF(uint8_t pan)
{
  return g_RfCfgParams[pan].SpreadingFactor;
}

/**
* @brief Get the current CRC check setting
*/
uint8_t PAN_GetCRC(uint8_t pan)
{
  return g_RfCfgParams[pan].CrcMode;
}

/**
* @brief Get the current coding rate setting
*/
uint8_t PAN_GetCR(uint8_t pan)
{
    return g_RfCfgParams[pan].CodingRate;
}

/**
* @brief Get the current sync word setting
*/
uint8_t PAN_GetSyncWord(uint8_t pan)
{
  return g_RfCfgParams[pan].SyncWord;
}

/**
* @brief Get the current transmit mode setting
*/
uint8_t PAN_GetLDR(uint8_t pan)
{
  return g_RfCfgParams[pan].LowDatarateOptimize;
}
...
*/
uint32_t PAN_GetTxTimeMs(uint8_t pan, uint8_t Size)
{
  ...
  sf = PAN_GetSF(pan);
  cr = PAN_GetCR(pan);
  bw = PAN_GetBandWidth(pan);
  ldr = PAN_GetLDR(pan);
    PreambleLen = PAN_GetPreamLen(pan);
...
*/
void PAN_EnableMapm(uint8_t pan)
{
  PAN_SetPageRegBits(pan, 1, 0x38, 0x01);      /* Enable mapm mode */
  PAN_WritePageRegBits(pan, 0, 0x58, 0, 0x40); /* Enable mapm interrupt */
}

/**
* @brief Disable MPM mode
*/
void PAN_DisableMapm(uint8_t pan)
{
  PAN_ResetPageRegBits(pan, 1, 0x38, 0x01);    /* Disable mapm mode */
  PAN_WritePageRegBits(pan, 0, 0x58, 1, 0x40); /* Disable mapm interrupt */
}
...
*/
void PAN_ConfigMapm(uint8_t pan, PAN_MapmCfg_t *pMapmCfg)
{
  ...
  reg_fn = (fn_h << 4) + fn_l;
  PAN_WritePageReg(pan, 1, 0x3D, reg_fn);                            /* set the number of fields */
  PAN_WritePageRegBits(pan, 1, 0x37, pMapmCfg->fnm, 0x80 | 0x40);    /* set the unit code word of the field counter represents several fields */
  PAN_WritePageRegBits(pan, 1, 0x38, pMapmCfg->gfs, 0x02);           /* set the last group function selection */
  PAN_WritePageRegBits(pan, 1, 0x38, pMapmCfg->gn - 1, 0x08 | 0x04); /* set the number of groups in Field */
  PAN_WritePageReg(pan, 1, 0x3B, pMapmCfg->pg1);                     /* set the number of Preambles in first groups */
  /* set the number of preambles for groups other than the first group */
  PAN_WritePageReg(pan, 1, 0x3C, pMapmCfg->pgn);
  /* set the number of preamble between the last group and the sync word */
  PAN_WritePageRegBits(pan, 1, 0x39, (uint8_t)(pMapmCfg->pn >> 8), 0x0F);
  PAN_WritePageReg(pan, 1, 0x3A, (uint8_t)(pMapmCfg->pn));
}
...
*/
void PAN_SetMapmAddr(uint8_t pan, uint8_t *MapmAddr, uint8_t AddrWidth)
{
  PAN_WritePageRegs(pan, 1, 0x3E, MapmAddr, AddrWidth);
}
...
*/
void PAN_StartTxContinuousWave(uint8_t pan)
{
  PAN_WriteReg(pan, 0x02, PAN_STATE_STB3);   /* Set the chip to idle state. */
  PAN_WritePageReg(pan, 0, 0x58, 0x00);     /* Disable all RF interrupts */
  PAN_SetTxMode(pan, PAN_TX_MODE_CONTINOUS); /* Set continuous transmit mode */
  PAN_WritePageReg(pan, 1, 0x0C, 1);        /* Set transmit data length to 1 byte */
  PAN_TurnonPA(pan);                      /* Enable the PA before transmitting data */
  PAN_WriteReg(pan, 0x02, PAN_STATE_TX);     /* Set the chip to transmit mode */
  PAN_WriteReg(pan, 0x01, 0xFF);            /* 0x01 is the FIFO register address. After writing data, start transmitting the carrier on the rising edge of CS */
  g_RfOperatetate[pan] = PAN_STATE_TX;
}
...
*/
void PAN_StopTxContinuousWave(uint8_t pan)
{
  PAN_WriteReg(pan, 0x02, PAN_STATE_STB3); /* Set the chip to idle state */
  PAN_TurnoffPA(pan);                   /* Turn off the PA after transmission is complete */
  PAN_WritePageReg(pan, 0, 0x58, 0x0F);   /* Restore the RF default interrupt */
  g_RfOperatetate[pan] = PAN_STATE_STB3;
}
...
*/
void PAN_IRQ_Process(uint8_t pan)
{
  if (CHECK_PAN_IRQ(pan)) /* RF interrupt detected, a high level indicates an interrupt */
  {
    uint8_t IRQFlag;

    IRQFlag = PAN_GetIRQFlag(pan);    /* Get interrupt flags */
    if (IRQFlag & PAN_IRQ_TX_DONE) /* Transmit complete interrupt */
    {
      PAN_TurnoffPA(pan);                /* Turn off PA after transmission completes */
      PAN_ClrIRQFlag(pan, PAN_IRQ_TX_DONE); /* Clear transmit complete interrupt flag */
      IRQFlag &= ~PAN_IRQ_TX_DONE;
    }
    if (IRQFlag & PAN_IRQ_RX_DONE) /* Receive complete interrupt */
    {
      g_RfRxPkt[pan].Snr = PAN_GetPktSnr();   /* Get the SNR value of the received packet */
      g_RfRxPkt[pan].Rssi = PAN_GetPktRssi(); /* Get the RSSI value of the received packet */
      /* Get the received data and length */
      g_RfRxPkt[pan].RxLen = PAN_GetRecvPayload((uint8_t *)g_RfRxPkt[pan].RxBuf);
      PAN_ClrIRQFlag(pan, PAN_IRQ_RX_DONE); /* Clear the receive complete interrupt flag */
    ...
    if (IRQFlag & PAN_IRQ_MAPM_DONE) /* MAPM receive complete interrupt */
    {
      ...
      g_RfRxPkt[pan].MapmRxBuf[g_RfRxPkt[pan].MapmRxIndex++] = MapmAddr;
      PAN_ClrIRQFlag(pan, PAN_IRQ_MAPM_DONE); /* Clear the MAMP receive completion interrupt flag */
      IRQFlag &= ~PAN_IRQ_MAPM_DONE;
    }
    if (IRQFlag & PAN_IRQ_CRC_ERR) /* CRC error interrupt */
    {
      PAN_ClrIRQFlag(pan, PAN_IRQ_CRC_ERR); /* Clear the CRC error interrupt flag */
    ...
    if (IRQFlag & PAN_IRQ_RX_TIMEOUT) /* Receive timeout interrupt */
    {
      ...
      PAN_ClrIRQFlag(pan, PAN_IRQ_RX_TIMEOUT); /* Clear the receive timeout interrupt flag */
    }
    if (IRQFlag) PAN_ClrIRQFlag(pan, IRQFlag); /* Clear the remaining interrupt flag */
  }
}
