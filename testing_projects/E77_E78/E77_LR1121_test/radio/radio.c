		...
		if(sweepflag[radio]) 
		{
			...
			if(sweeprx[radio])
			{
				...
				if(rssi > rssi_tr[radio])
				{
					...
					printf("@R%d,%d,%.1f\r\n",radio,currfreq[radio] / 1000,rssi); //temporary; send to buffer instead
...
void radio_initconfigs(void)
{
  ...
  globalrfconfig.LBconfig.rffreq = 868000000;
  globalrfconfig.LBconfig.txpower = 16;
  ...
  globalrfconfig.HBconfig.rffreq = 430000000;
  ...
  globalrfconfig.HBconfig.bw = LORA_BW_62p5; //bandwidth
...
bool radio_init(uint8_t radio) 
{
  switch(radio)
  {
    ...
    return true;
		...
		currfreq[1] = globalrfconfig.HBconfig.rffreq;
...
uint8_t radio_readregister(uint8_t radio,uint32_t reg)
{
  uint8_t regval;
	...
	switch(radio)
  {
    case RADIO_L: //internal SX1262
		...
		case RADIO_H:
		LR112X_ReadRegMem32(0,reg, &regval,1);	
		return regval;
...
void radio_writeregister(uint8_t radio,uint32_t reg,uint8_t value)
{
  switch(radio)
  {
    case RADIO_L: //internal SX1262
		SX126X_writeReg(0,(uint16_t)reg,value);
		break;
		
		case RADIO_H:
		LR112X_WriteRegMem32(0,reg,&value,1);
		break;
