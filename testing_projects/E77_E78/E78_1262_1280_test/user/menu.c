...
MENU_ITEM(m_timesettings,		m_additional, 			m_radiosettings, 				NULL_MENU, 					NULL_MENU,			NULL, 						NULL,											"\2 Date/time      ");
...
MENU_ITEM(m1_rssi, 					m1_exit,						m1_step,	 							NULL_MENU,					NULL_MENU, 			NULL, 						b1_rssi,				 					"\4 RSSI threshold ");
MENU_ITEM(m1_exit, 					NULL_MENU,					m1_rssi,	 							m_band1,						NULL_MENU, 			NULL, 						b1_exit,				 					"\4 Exit           ");
...
MENU_ITEM(m2_rssi, 					m2_exit,						m2_step,	 							NULL_MENU,					NULL_MENU, 			NULL, 						b2_rssi,				 					"\4 RSSI threshold ");
MENU_ITEM(m2_exit, 					NULL_MENU,					m2_rssi,	 							m_band2,						NULL_MENU, 			NULL, 						b2_exit,				 					"\4 Exit           ");
...
MENU_ITEM(m3_rssi, 					m3_exit,						m3_step,	 							NULL_MENU,					NULL_MENU, 			NULL, 						b3_rssi,				 					"\4 RSSI threshold ");
MENU_ITEM(m3_exit, 					NULL_MENU,					m3_rssi,	 							m_band3,						NULL_MENU, 			NULL, 						b3_exit,				 					"\4 Exit           ");
...
void b1_startfreq(void)
{
	...
	uint32_t freq = globalrfconfig.LBconfig.rffreq / 1000;
	cls();
	...
	Key = K_NONE;
	globalrfconfig.LBconfig.rffreq = freq * 1000;
	menu_display(&m1_start,0);
	menu_display(&m1_stop,1);
	menu_display(&m1_step,1);
	menu_display(&m1_rssi,1);
...
void b1_stopfreq(void)
{
	//read stopfreq. from radioconfig
	//key_up + 1 freq.step
	//key_down -1 freq.step
	//check for limits
	//call freq. display
...
void b1_freqstep(void)
{
	//read freqstep. from radioconfig
	//key_up + 10 kHz
	//key_down -10 kHz
	//check for limits
	//call step. display
...
void b2_startfreq(void)
{
	uint8_t temp_key;
	//read start, freq. from radioconfig
	uint32_t freq = globalrfconfig.MBconfig.rffreq / 1000;
	cls();
	...
	Key = K_NONE;
	globalrfconfig.MBconfig.rffreq = freq * 1000;
	menu_display(&m2_start,0);
	menu_display(&m2_stop,1);
	menu_display(&m2_step,1);
	menu_display(&m2_rssi,1);
...
void b3_startfreq(void)
{
	uint8_t temp_key;
	//read start, freq. from radioconfig
	uint32_t freq = globalrfconfig.HBconfig.rffreq / 1000;
	cls();
	...
	Key = K_NONE;
	globalrfconfig.HBconfig.rffreq = freq * 1000;
	menu_display(&m3_start,0);
	menu_display(&m3_stop,1);
	menu_display(&m3_step,1);
	menu_display(&m3_rssi,1);
