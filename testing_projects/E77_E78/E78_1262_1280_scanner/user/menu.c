...
void b3_enter(void);
void b3_startfreq(void);
void b3_stopfreq(void);
void b3_freqstep(void);
void b3_rssi(void);
void b3_act(void);
void b3_exit(void);
...
gpio_t* KEY_PORT[KEY_COUNT] = {K_UP_PORT,K_DOWN_PORT,K_ENTER_PORT};
const uint8_t KEY_PIN[KEY_COUNT] = {K_UP_PIN,K_DOWN_PIN,K_ENTER_PIN};
...
MENU_ITEM(m_band2, 					m_band3, 						m_band1,	 							NULL_MENU,					m2_start, 			NULL, 						b2_enter,				 					"\2 Band 2 settings");
MENU_ITEM(m_band3, 					m_radioexit,				m_band2,	 							NULL_MENU,					m3_start, 			NULL, 						b3_enter,				 					"\3 Band 3 settings");
MENU_ITEM(m_radioexit,			NULL_MENU,					m_band3,	 							m_radiosettings,		NULL_MENU, 			NULL, 						radio_settings_exit,			"\4 Save and exit  ");
...
MENU_ITEM(m3_start, 				m3_stop, 						NULL_MENU, 							NULL_MENU,					NULL_MENU, 			NULL, 						b3_startfreq,							"\1 Start Frequency");
MENU_ITEM(m3_stop, 					m3_step, 						m3_start,	 							NULL_MENU,					NULL_MENU, 			NULL, 						b3_stopfreq,		 					"\2 Stop Frequency ");
MENU_ITEM(m3_step, 					m3_rssi,						m3_stop,	 							NULL_MENU,					NULL_MENU, 			NULL, 						b3_freqstep,		 					"\3 Frequency Step ");
MENU_ITEM(m3_rssi, 					m3_act,							m3_step,	 							NULL_MENU,					NULL_MENU, 			NULL, 						b3_rssi,				 					"\4 RSSI threshold ");
MENU_ITEM(m3_act, 					m3_exit,						m3_rssi,	 							NULL_MENU,					NULL_MENU, 			NULL, 						b3_act,	 				 					"\4 Activation     ");
MENU_ITEM(m3_exit, 					NULL_MENU,					m3_act,		 							m_band3,						NULL_MENU, 			NULL, 						b3_exit,				 					"\4 Exit           ");
...
void b2_startfreq(void)
{
	uint8_t temp_key;
	//read start freq. from radioconfig
	uint32_t freq = globalrfconfig.MBconfig.startfreq / 1000;
	cls();
	...
	//key_up + 1 freq.step
		if(temp_key == K_UP)
		{
			freq += globalrfconfig.MBconfig.freqstep / 1000; //temporary
			if(freq >= globalrfconfig.MBconfig.stopfreq /1000) freq = globalrfconfig.MBconfig.stopfreq - 2 * (globalrfconfig.MBconfig.freqstep / 1000);
			if(freq > MB_FREQ_MAX / 1000) freq = MB_FREQ_MAX / 1000;
		}
	//key_down -1 freq.step
		if(temp_key == K_DOWN)
		{
			freq -= globalrfconfig.MBconfig.freqstep / 1000;
			if(freq < MB_FREQ_MIN / 1000) freq = MB_FREQ_MIN / 1000;
		}
	//call freq. display
		display_start_freq(freq);
	}
	Key = K_NONE;
	globalrfconfig.MBconfig.startfreq = freq * 1000;
	menu_display(&m2_start,0);
	menu_display(&m2_stop,1);
	menu_display(&m2_step,1);
	menu_display(&m2_rssi,1);
...
void b2_stopfreq(void)
{
	uint8_t temp_key;
	//read stop freq. from radioconfig
	uint32_t freq = globalrfconfig.MBconfig.stopfreq / 1000;
	cls();
	GUI_ShowString(0,16,"BAND 2",16,1);
	GUI_ShowString(0,48,"ENTER to save",16,1);
	while(Key != K_ENTER)
	...
	//key_down -1 freq.step
		...
		{
			freq -= globalrfconfig.MBconfig.freqstep / 1000;
			if(freq <= globalrfconfig.MBconfig.startfreq /1000) freq = globalrfconfig.MBconfig.startfreq + 2 * (globalrfconfig.MBconfig.freqstep / 1000);
			if(freq <= MB_FREQ_MIN / 1000) freq = MB_FREQ_MIN / 1000 + globalrfconfig.MBconfig.freqstep / 1000;
		}
	//call freq. display
		display_stop_freq(freq);
	}
	Key = K_NONE;
	globalrfconfig.MBconfig.stopfreq = freq * 1000;
...
void b2_freqstep(void)
{
	...
	uint32_t step = globalrfconfig.MBconfig.freqstep / 1000;
	cls();
	GUI_ShowString(0,16,"BAND 2",16,1);
	GUI_ShowString(0,48,"ENTER to save",16,1);
	while(Key != K_ENTER)
	...
	Key = K_NONE;
	globalrfconfig.MBconfig.freqstep = step * 1000;
	menu_display(&m2_start,1);
	menu_display(&m2_stop,1);
...
void b2_rssi(void)
{
	uint8_t temp_key;
	//read rssi from radioconfig
	int8_t tr = globalrfconfig.MBconfig.rssithreshold;
	cls();
	GUI_ShowString(0,16,"BAND 2",16,1);
	GUI_ShowString(0,48,"ENTER to save",16,1);
	while(Key != K_ENTER)
	...
	Key = K_NONE;
	globalrfconfig.MBconfig.rssithreshold = tr;
	menu_display(&m2_start,1);
	menu_display(&m2_stop,1);
	menu_display(&m2_step,1);
...
void b2_act(void)
{
	uint8_t temp_key;
	//read from radioconfig
	bool act = globalrfconfig.MBconfig.scan_active;
	cls();
	GUI_ShowString(0,16,"BAND 2",16,1);
	GUI_ShowString(0,48,"ENTER to save",16,1);
	while(Key != K_ENTER)
	...
	Key = K_NONE;
	globalrfconfig.MBconfig.scan_active = act;
	menu_display(&m2_start,1);
	menu_display(&m2_stop,1);
	menu_display(&m2_step,1);
	menu_display(&m2_rssi,0);
...
{
	Menu_Navigate(MENU_CHILD);
	menu_display(&m3_start,0);
	menu_display(&m3_stop,1);
	menu_display(&m3_step,1);
	menu_display(&m3_rssi,1);
}
...
void b3_startfreq(void)
{
	...
	cls();
	GUI_ShowString(0,16,"BAND 3",16,1);
	GUI_ShowString(0,48,"ENTER to save",16,1);
	while(Key != K_ENTER)
	{
		temp_key = Key;
...
void b3_stopfreq(void)
{
	...
	menu_display(&m3_start,1);
	menu_display(&m3_stop,0);
	menu_display(&m3_step,1);
	menu_display(&m3_rssi,1);
	Menu_Navigate(&m3_stop);
}
...
void b3_freqstep(void)
{
	...
	menu_display(&m3_start,1);
	menu_display(&m3_stop,1);
	menu_display(&m3_step,0);
	menu_display(&m3_rssi,1);
	Menu_Navigate(&m3_step);
}
...
void b3_rssi(void)
{
	...
	menu_display(&m3_stop,1);
	menu_display(&m3_step,1);
	menu_display(&m3_rssi,0);
	Menu_Navigate(&m3_rssi);
}
