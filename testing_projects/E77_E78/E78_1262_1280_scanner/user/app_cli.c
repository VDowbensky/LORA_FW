...
CommandEntry_t commands[] =
  {
    ...
    COMMAND_ENTRY("GET_SCANPARAMS", "w", cli_getscanparams, "Get scanning parameters"),
    COMMAND_ENTRY("SET_SCANPARAMS", "wwwwww", cli_setscanparams, "Set scanning parameters"),
		COMMAND_ENTRY("GET_SCANS","",cli_getactivescans,""),
		COMMAND_ENTRY("SET_SCANS","www",cli_setactivescans,""),
...
void cli_getscanparams(int argc, char **argv)
{
	...
	if(radio > 2)
	{
		printf("GET_SCANPARAMS: INVALID RADIO\r\n");
		return;
	}
	...
	else if(radio == RADIO_M) printf("GET_SCANPARAMS: %u,%u,%u,%u,%u,%d\r\n",radio,globalrfconfig.MBconfig.startfreq / 1000,
																																				globalrfconfig.MBconfig.stopfreq / 1000,
																																				globalrfconfig.MBconfig.freqstep / 1000,
...
void cli_setscanparams(int argc, char **argv)
{
	...
	{
		globalrfconfig.MBconfig.startfreq = start * 1000;
		globalrfconfig.MBconfig.stopfreq = stop * 1000;
		globalrfconfig.MBconfig.freqstep= step * 1000;
		globalrfconfig.MBconfig.scantimems = time;
...
void cli_getactivescans(int argc, char **argv)
{
	...
	if(globalrfconfig.MBconfig.scan_active == true) s1 = 1;
	if(globalrfconfig.HBconfig.scan_active == true) s2 = 1;
	printf("GET_SCANS: %d,%d,%d\r\n",s0,s1,s2);
...
void cli_setactivescans(int argc, char **argv)
{
	...
	uint8_t s2 = ciGetUnsigned(argv[3]);
	...
	else globalrfconfig.MBconfig.scan_active = false;
