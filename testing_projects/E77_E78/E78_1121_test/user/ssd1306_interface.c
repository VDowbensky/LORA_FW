...
void SSD1306_interface_init(void)
{
	...
	rcc_enable_peripheral_clk(SSD1306_RCC_PERIPHERAL, true);
	...
	init_struct.ssp_clk_pol   = SPI_CLK_POLARITY_LOW; //SPI_CLK_POLARITY_HIGH;
	init_struct.ssp_clk_phase = SPI_CLK_PHASE_1EDGE; //SPI_CLK_PHASE_2EDGE;
	init_struct.ssp_dma_tx_en = DISABLE;
	init_struct.ssp_dma_rx_en = DISABLE;
	ssp_init(SSD1306_SSP, &init_struct);
	ssp_cmd(SSD1306_SSP, ENABLE);
}
...
{
	return spi_transfer(SSD1306_SSP,b);
}
